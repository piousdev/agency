...
- [ ] **For apps/web work**: Read `apps/web/ARCHITECTURE.md` first (Server-First patterns)
...
### Server-First Architecture (apps/web)

**MANDATORY**: Read `apps/web/ARCHITECTURE.md` before implementing web features.

Key patterns (see `apps/web/ARCHITECTURE.md` for details):

- Server Components: Default for all pages (no "use client" unless interactive)
- Server Actions: All mutations and form submissions
- Client components: Only for interactivity (forms, dropdowns, modals)
- Auth session data: Better-Auth useSession (NOT Zustand/TanStack Query)
- Auth UI state: Zustand for preferences/temporary state only
- Performance: Cookie cache enabled (5min, 80% DB load reduction)

Reference implementations:

- Protected page: `apps/web/src/app/dashboard/page.tsx`
- Server Action form: `apps/web/src/app/accept-invite/[token]/`
- Auth hooks: `apps/web/src/lib/hooks/use-auth.ts`
- UI state: `apps/web/src/lib/stores/auth-ui.ts`
