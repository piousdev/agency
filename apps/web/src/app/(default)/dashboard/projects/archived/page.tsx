...
export default async function ArchivedProjectsPage() {
  ...
  // Filter for archived/completed projects (delivered, on_hold)
  const projects = projectsResponse.data.filter((p) => ['delivered', 'on_hold'].includes(p.status));
  ...
  return (
    <div className="container mx-auto py-8 space-y-6 max-w-full overflow-hidden px-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold">Archived Projects</h1>
          <p className="text-muted-foreground mt-2">Completed, paused, or archived projects</p>
        </div>
      ...
      {projects.length > 0 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            Showing <span className="font-medium">{projects.length}</span> archived project
            {projects.length !== 1 ? 's' : ''}
          </p>
        </div>
      )}
