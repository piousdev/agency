...
export function TableView({
  ...
  const handleTransition = async (request: Request, newStage: RequestStage, reason?: string) => {
    const oldStage = request.stage as RequestStage;
  ...
  const handleStageClick = (request: Request, newStage: RequestStage) => {
    if (newStage === 'on_hold') {
      ...
      handleTransition(request, newStage);
    }
  };
  ...
  const columns: ColumnDef<Request>[] = [
    ...
    },
    {
      ...
      header: 'Stage',
      cell: ({ row }) => {
        const stage = row.original.stage as RequestStage;
  ...
  const renderRowActions = (row: Row<Request>) => {
    ...
    const currentStage = request.stage as RequestStage;
              ...
              <DropdownMenuItem
                ...
                onClick={async () => {
                  for (const row of selectedRows) {
                    const request = row.original;
                    const validStages = VALID_TRANSITIONS[request.stage as RequestStage] || [];
