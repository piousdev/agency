...
export function BulkConfirmationDialog({
  ...
  const getTitle = () => {
    if (action.type === 'transition') {
      return `Move ${selectedCount} request${selectedCount !== 1 ? 's' : ''} to ${REQUEST_STAGE_LABELS[action.stage]}?`;
    }
    return `Assign ${action.pmName} to ${selectedCount} request${selectedCount !== 1 ? 's' : ''}?`;
  };
  ...
  const getDescription = () => {
    if (action.type === 'transition') {
      ...
      return `This will change the stage of ${selectedCount} selected request${selectedCount !== 1 ? 's' : ''} to "${REQUEST_STAGE_LABELS[action.stage]}".`;
    }
    return `This will assign ${action.pmName} as the PM for ${selectedCount} selected request${selectedCount !== 1 ? 's' : ''}.`;
  };
