...
import { RequestCard } from '@/components/dashboard/business-center/intake/request-card';
import { BulkActionsBar } from '@/components/dashboard/business-center/intake/bulk-actions-bar';
import {
  BulkConfirmationDialog,
  type BulkActionType,
} from '@/components/dashboard/business-center/intake/bulk-confirmation-dialog';
import { ActionSheet } from '@/components/dashboard/business-center/intake/action-sheet';
import { FilterPopover } from '@/components/dashboard/business-center/intake/filter-popover';
import { KanbanView } from '@/components/dashboard/business-center/intake/kanban-view';
import { TableView } from '@/components/dashboard/business-center/intake/table-view';
import { Pagination } from '@/components/dashboard/business-center/intake/pagination';
import {
...
export function IntakeClient({
  ...
  const { isConnected, lastEventTimestamp } = useIntakeSocket({
    enabled: true,
    ...
    onEstimated: (payload) => {
      toast.info(`Request estimated: ${payload.requestTitle}`, {
        description: `${payload.storyPoints} points (${payload.confidence}) by ${payload.estimatorName}`,
        duration: 4000,
      });
      handleSocketEvent();
    },
      ...
      if (pendingAction.type === 'transition') {
        ...
        if (result.success) {
          ...
          if (failedIds.length === 0) {
            toast.success(
              `${successIds.length} request${successIds.length !== 1 ? 's' : ''} moved to ${REQUEST_STAGE_LABELS[pendingAction.stage]}`
            );
          } else {
            toast.warning(`${successIds.length} succeeded, ${failedIds.length} failed`, {
        ...
        if (result.success) {
          ...
          if (failedIds.length === 0) {
            toast.success(
              `${pendingAction.pmName} assigned to ${successIds.length} request${successIds.length !== 1 ? 's' : ''}`
            );
          } else {
            toast.warning(`${successIds.length} succeeded, ${failedIds.length} failed`, {
              description: failedIds.map((f) => f.error).join(', '),
    ...
    (requests: Request[]) => {
      return requests.filter((request) => {
        ...
        if (
          ...
          !filters.stage.includes(request.stage as (typeof filters.stage)[number])
        ) {
          return false;
        }
  ...
  const stages: Array<RequestStage | 'all'> = [
    'all',
    'in_treatment',
    'on_hold',
    'estimation',
          ...
          {stages.map((stage) => (
            <TabsTrigger key={stage} value={stage} className="gap-2">
              ...
              <Badge variant="secondary" className="ml-1">
                {stage === 'all'
                  ? Object.values(initialStageCounts).reduce((a, b) => a + b, 0)
