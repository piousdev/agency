...
export function SwipeableCard({
  ...
  rightAction = { label: 'Select', icon: <IconCheck className="h-5 w-5" />, color: 'primary' },
  disabled = false,
  className,
}: SwipeableCardProps) {
    ...
    <div className={cn('relative overflow-hidden rounded-lg', className)}>
      ...
      {onSwipeLeft && (
        ...
        >
          <div
            ...
            style={{ opacity: leftProgress, transform: `scale(${0.8 + leftProgress * 0.2})` }}
          >
            {leftAction.icon}
      ...
      {onSwipeRight && (
        ...
        >
          <div
            className="flex flex-col items-center gap-1"
            style={{ opacity: rightProgress, transform: `scale(${0.8 + rightProgress * 0.2})` }}
          >
            {rightAction.icon}
