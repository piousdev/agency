...
function getAgingStatus(stage: string, stageEnteredAt: string): 'normal' | 'warning' | 'critical' {
  // Check if stage is a valid key before accessing
  if (!(stage in STAGE_THRESHOLDS)) return 'normal';
...
function formatTimeInStage(stageEnteredAt: string): string {
  ...
  if (diffDays > 0) {
    return `${String(diffDays)} day${diffDays !== 1 ? 's' : ''}, ${String(diffHours % 24)} hour${diffHours % 24 !== 1 ? 's' : ''}`;
  }
  if (diffHours > 0) {
    return `${String(diffHours)} hour${diffHours !== 1 ? 's' : ''}`;
  }
